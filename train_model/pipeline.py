...
class Pipeline:
    ...
    def load_data(self, img_dimension=(5, 5), n_channels=3, target_dim=[None, 1]):
        self.x = tf.placeholder(
            "float32",
            shape=[None, img_dimension[0], img_dimension[1], 3],
            name="input_images",
        )
    ...
    def train(self, x_train, y_train,z_train, keep_prob=0.5):
    ...
    def validation(self, x_val, y_val,z_val):
        ...
        try:
            while True:
                ...
                val_loss, _, _, sm = self.sess.run(
                    [self.loss, self.images, self.target, self.summaries],
                    feed_dict={self.model.keep_prob: 1, self.model.is_training: False},
                )
    ...
    def test(self, x_test, y_test,z_test):
        ...
        try:
            while True:
                # Aqui no esta reutilizando los batches
                test_loss, _, _ = self.sess.run(
                    [self.loss, self.images, self.target],
                    feed_dict={self.model.keep_prob: 1, self.model.is_training: False},
                )
        ...
        with open(self.save_path + "/setup.txt", "a") as self.out:
            self.out.write(
                "best model found in iter: " + str(self.best_model_epoch) + "\n"
            )
        return np.mean(epoch_test_loss)

    def fit(
        self, x_train, y_train,z_train, x_val, y_val,z_val, n_epochs=10, stop_step=20, keep_prob=0.5
    ):
